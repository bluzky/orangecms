<svelte:options immutable={true} /><script>import { icons } from './icons';
...
export let locale;
export let actions;
export let rightAfferentActions;
...
><div class="bytemd-toolbar-left">{#if split}{#each actions as item, index}{#if item.handler}<div
      ...
      >{locale.write}</div><div
        ...
        class="bytemd-toolbar-tab"
        class:bytemd-toolbar-tab-active={activeTab === 'preview'}
      >{locale.preview}</div>{/if}</div><div class="bytemd-toolbar-right">{#each rightActions as item, index}{#if !item.hidden}<div
